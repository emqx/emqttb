...
%%--------------------------------------------------------------------
-module(emqttb_scenario_sub_unsub_flapping).
...
%% This scenario consists of clients connecting once, then entering a cycle of subscribing
%% to a topic, waiting for a period of time, then unsubscribing and subscribing again.

%%================================================================================
%% Type declarations
%%================================================================================

-define(GROUP, sub_unsub_flapping).
...
model() ->
  #{ topic =>
   ...
   , conninterval =>
       {[value, cli_param, autorate],
        #{ oneliner => "Client connection interval"
         ...
         , cli_short => $I
         , autorate_id => 'sub_unsub_flapping/conninterval'
         }}
   , n_clients =>
       {[value, cli_param],
   ...
   , flap_period =>
       {[value, cli_param],
        #{ oneliner => "Subscribe/unsubscribe period"
         , type => emqttb:duration_ms()
         , default_str => "1s"
         , cli_operand => "flap-period"
         , cli_short => $f
         }}
   , metrics =>
       emqttb_behavior_sub:model('sub_flapping/sub_unsub')
   }.

initial_config() ->
  emqttb_conf:string2patch("@a -a sub_unsub_flapping/conninterval --pvar '[scenarios,sub_unsub_flapping,{},metrics,conn_latency,pending]' --olp").
