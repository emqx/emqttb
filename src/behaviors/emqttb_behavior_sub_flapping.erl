...
%%--------------------------------------------------------------------
-module(emqttb_behavior_sub_flapping).
...
-type config() :: #{ topic           := binary()
                   ...
                   , flap_period     := emqttb:duration_ms()
                   }.
...
init(SubOpts0 = #{ topic := T
  ...
  erlang:send_after(FlapPeriod, self(), unsubscribe),
  Conn.

handle_message(Conf, Conn, {publish, Msg = #{client_pid := Pid}}) when Pid =:= Conn ->
  emqttb_msg_payload:on_receive_message(Conf, Msg),
  {ok, Conn};
handle_message(Conf, Conn, unsubscribe) ->
  #{ topic := T
   , qos := QoS
   , sub_opstat := SubOpstat
   , flap_period := FlapPeriod
   } = Conf,
  emqttb_metrics:call_with_counter(SubOpstat, emqtt, unsubscribe, [Conn, emqttb_worker:format_topic(T)]),
  emqttb_metrics:call_with_counter(SubOpstat, emqtt, subscribe, [Conn, emqttb_worker:format_topic(T), QoS]),
  erlang:send_after(FlapPeriod, self(), unsubscribe),
  {ok, Conn};
